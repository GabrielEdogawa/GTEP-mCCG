...
Lower(rowset)..          eta =g= sum(s, sum(eg, epm(eg, s, rowset) * EGDATA(eg, 'OC')) + sum(cg, cpm(cg, s, rowset) * CGDATA(cg, 'OC')) +
                         sum(d, rm(d, s, rowset) * PC));
...
eleq1(el, s, rowset)..              efm(el, s, rowset) =g= (sum(n$mapESL(el,n), thetam(n, s, rowset)) -
                                 sum(n$mapERL(el,n), thetam(n, s, rowset))) / ELDATA(el, 'X') - M * AELm(el, rowset);
eleq2(el, s, rowset)..              efm(el, s, rowset) =l= (sum(n$mapESL(el,n), thetam(n, s, rowset)) -
                                 sum(n$mapERL(el,n), thetam(n, s, rowset))) / ELDATA(el, 'X') + M * AELm(el, rowset);
ellim1(el, s, rowset)..             efm(el, s, rowset) =g= - (1-AELm(el, rowset)) * ELDATA(el, 'FLmax');
ellim2(el, s, rowset)..             efm(el, s, rowset) =l= (1-AELm(el, rowset)) * ELDATA(el, 'FLmax');

cleq1(cl, s, rowset)..              cfm(cl, s, rowset) =g= (sum(n$mapCSL(cl,n), thetam(n, s, rowset)) -
                                 sum(n$mapCRL(cl,n), thetam(n, s, rowset))) / CLDATA(cl, 'X') - M * (1-(1-ACLm(cl, rowset)) * xl(cl));
cleq2(cl, s, rowset)..              cfm(cl, s, rowset) =l= (sum(n$mapCSL(cl,n), thetam(n, s, rowset)) -
                                 sum(n$mapCRL(cl,n), thetam(n, s, rowset))) / CLDATA(cl, 'X') + M * (1-(1-ACLm(cl, rowset)) * xl(cl));
cllim1(cl, s, rowset)..             cfm(cl, s, rowset) =g= - CLDATA(cl, 'FLmax')  * (1-ACLm(cl, rowset)) * xl(cl);
cllim2(cl, s, rowset)..             cfm(cl, s, rowset) =l= CLDATA(cl, 'FLmax')  * (1-ACLm(cl, rowset)) * xl(cl);

eglim1(eg, s, rowset)..             epm(eg, s, rowset) =l= (1-AEGm(eg, rowset)) * EGDATA(eg, 'Pmax');
eglim2(eg, s, rowset)..             epm(eg, s, rowset) =g= (1-AEGm(eg, rowset)) * EGDATA(eg, 'Pmin');
cglim1(cg, s, rowset)..             cpm(cg, s, rowset) =l= CGDATA(cg, 'Pmax') * (1-ACGm(cg, rowset)) * xg(cg);
cglim2(cg, s, rowset)..             cpm(cg, s, rowset) =g= CGDATA(cg, 'Pmin') * (1-ACGm(cg, rowset)) * xg(cg);

lslimm(n, s, rowset)..              sum(d$mapD(d,n), rm(d, s, rowset)) =l= sum(d$mapD(d,n), DDATA(d,s));
