...
xglim..         sum(g$cg(g), xg(g)) =l= 3;
xllim..         sum(l$cl(l), xl(l)) =l= 3;
...
Scalar
         PC      Penalty Cost for imbalanced demand    /12000/
...
Eq_dual11..              sum(g, AG(g)) =l= 2;
Eq_dual12..              sum(l, AL(l)) =l= 2;
...
Model dualsp / obj_dualsp, gendual, linedual, thetadual, rdual, Eq_dual51, Eq_dual52, Eq_dual61,
                 ...
                 Eq_dual101, Eq_dual102, Eq_dual11, Eq_dual12, Eq_dual53, Eq_dual63, Eq_dual73, Eq_dual83, Eq_dual93, Eq_dual103 /;
...
xglim0..                       sum(g$cg(g), xg(g)) =l= 3;
xllim0..                       sum(l$cl(l), xl(l)) =l= 3;
...
Lineq3(l, columnset)$cl(l)..         f(l) =g= (sum(n$mapSL(l,n), theta(n)) - sum(n$mapRL(l,n), theta(n))) / LDATA(l, '1') - M * (1-xl(l));
Lineq4(l, columnset)$cl(l)..         f(l) =l= (sum(n$mapSL(l,n), theta(n)) - sum(n$mapRL(l,n), theta(n))) / LDATA(l, '1') + M * (1-xl(l));
Llim30(l, columnset)$cl(l)..         f(l) =g= - xl(l) * LDATA(l, '4');
Llim40(l, columnset)$cl(l)..         f(l) =l= xl(l) * LDATA(l, '4');
